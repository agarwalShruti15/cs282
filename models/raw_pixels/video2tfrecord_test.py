    ...
    for f in range(frame_count):
      if math.floor(f % steps) == 0 or take_all_frames:
        ...
        if frame is None and frames_counter < n_frames:
          ...
          image=paralell_frame(frame,image)
...
def paralell_frame(frame,image):
    ...
    while True:
        ...
        if len(pending) < threadn:
            ...
            #last_frame_time = t
            if threaded_mode:
                proc_image = pool.apply_async(proc_image, (frame,image))
            else:
                proc_image = proc_image(frame,image)
            pending.append(task)
        ch = cv.waitKey(1)
        if ch == ord(' '):
