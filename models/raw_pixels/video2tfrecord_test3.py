...
def convert_videos_to_tfrecord(source_path, destination_path,
  ...
  chunksize=len(filenames_split)%threadn
  ...
  for i, batch in enumerate(filenames_split):
    ...
    with Pool(processes=threadn) as pool:
        func = partial(convert_video_to_numpy, width=width, height=height,
                                  n_frames_per_video=n_frames_per_video,
                                  n_channels=n_channels,
                                  dense_optical_flow=dense_optical_flow)

        data=pool.map(func, batch,chunksize)
    ...
    func_par=partial(report_parallel,data_len=len(batch))
    for video in batch:
        pool.apply_async(convert_video_to_numpy,args=[filenames=video, width=width, height=height,
                                      n_frames_per_video=n_frames_per_video,
                                      n_channels=n_channels,
                                      dense_optical_flow=dense_optical_flow],callback=func_par)
    '''
#----------------------------END -----------
    '''
...
def video_file_to_ndarray(i, file_path, n_frames_per_video, height, width,
  ...
  sample_size=300 #To split video
  frame_list=list(np.arange(frame_count))
  ...
  frame_dic={}
  selec_img=[]
  video = np.zeros((len(sample_lists)*n_frames, height, width, n_channels), dtype=np.uint32)
  ...
  for i in range(len(sample_lists)):
      v_i=video[i*n_frames_per_video:(i+1)*n_frames_per_video]
      videos_ext.append(v_i)
  v=videos_ext.copy()
  cap.release()
  print("New video processed")
...
def convert_video_to_numpy(filenames, n_frames_per_video, width, height,
  ...
  for i in range(1):
    try:
      v = video_file_to_ndarray(i=i, file_path=filenames,
                                n_frames_per_video=n_frames_per_video,
